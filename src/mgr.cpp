...
Manager::Impl * Manager::Impl::init(const Config &cfg)
{
    ...
    for (int64_t i = 0; i < (int64_t)cfg.numWorlds; i++) {
        world_inits[i] = WorldInit {
            episode_mgr,
            int32_t(cfg.numRectanglesPerWorld),
            cfg.boundsMinX,
            cfg.boundsMaxX,
            cfg.boundsMinY,
            cfg.boundsMaxY,
...
MADRONA_EXPORT GPUTensor Manager::actionTensor() const
{
    ...
    return GPUTensor(dev_ptr, GPUTensor::ElementType::Float32,
        {impl_->cfg.numWorlds, impl_->cfg.numRectanglesPerWorld, 3},
         impl_->cfg.gpuID);
}
...
MADRONA_IMPORT GPUTensor Manager::numOverlapsTensor() const
{
    void *dev_ptr = impl_->mwGPU.getExported(3);

    return GPUTensor(dev_ptr, GPUTensor::ElementType::Int32,
        {impl_->cfg.numWorlds, impl_->cfg.numRectanglesPerWorld, 1},
         impl_->cfg.gpuID);
}
